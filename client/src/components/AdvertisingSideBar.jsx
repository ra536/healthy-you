...
const AdvertisingSideBar = (props) => {
  ...
  const determineProfile = (doctor) => {
    ...
    if (
      doctor.category?.includes("Gym") ||
      doctor.category?.includes("Personal Trainers")
    ) {
              ...
              <Container key={index}>
                <Row>
                  ...
                  <Col>
                    ...
                    <h6>
                      {featuredDoctor.category.map((category, index) => (
                        <React.Fragment key={index}>
                          <a
                            href={
                              "/results/" +
                              region +
                              "/?practice=&specialty=&location=&category=" +
                              category
                            }
                          >
                            {category}
                      ...
                      {featuredDoctor.specialty.map((specialty, index) => (
                        <React.Fragment key={index}>
                          <a
                            href={
                              "/results/" +
                    ...
                    <h6>
                      {featuredDoctor.practices
                        .map(
                          (practices) =>
                            `${practices.name} - ${practices.location}`
                        )
                        .join(", ")}{" "}
                    </h6>
                  </Col>
